...
- 当前状态：PLAN_APPROVED_R2 / **Issue 已置 `blocked`（巡检 20260822-2003，`--no-start`）；Coder lane 停机，不自动重试，模型路由待 dongsjoa 裁决（A 等恢复 / B 切 claude / C 退档 / D 暂挂）。leader 更正一条：option B **未获执行而非已证伪**（三次 run 仅 35s/35s/28s，对照 `cd0f9fa3` 698s，秒挂于脏会话重放，brief 大小未起作用）；9 次失败全归 runtime/infra，不计入 Coder、不进返修链。RED 仍未受理**（取证分支 `20260822-hd30-red-rescue-session-01a02a28` head `f90d64ee` 已使 ancestry 三项 exit 0，可达性阻塞解除；但 RED 仍不受理：`ddb9560b` 为零内容 merge（树与 `a17eab1d` 逐字节相同）、加绑条件 2 的 probe 只存在于人工署名且自标 NOT RED-verified 的 `f90d64ee`、本轮无任何 RED 实跑输出进入可读 ref。写集 leaf-path 经 leader 逐字符复核越界 0，转 CLEAN。抢救物内容质量合格——断言七槽位、entityId 语义、preference 闸门约束均已核对通过——缺的是提交面与实跑留痕）
